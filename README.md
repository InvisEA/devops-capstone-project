...
![Build Status](https://github.com/Imhopepus/devops-capstone-project/actions/workflows/ci-build.yaml/badge.svg)
...
This project was implemented in the IBM Cloud with OpenShift. 
You can initialize your environment with `bin/setup.sh` by sourcing it. (*Note: DO NOT run this program as a bash script. It sets environment variable and so must be sourced*):
...
Goal was to create microservice by implementing REST API's for `READ`, `UPDATE`, `DELETE`, and `LIST` while maintaining **95%** code coverage. In true **Test Driven Development** fashion, first tests were written and then was written the code to make them pass.
